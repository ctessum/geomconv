...
func polygon2geom(s shp.Polygon) geom.T {
	pg := new(geom.Polygon)
	pg.Rings = make([][]geom.Point, len(s.Parts))
	for i := 0; i < len(s.Parts); i++ {
		...
		for j := end - 1; j >= start; j-- {
			pg.Rings[i][j-start] = geom.Point(s.Points[j])
		}
	}
	return *pg
}
func polygonM2geom(s shp.PolygonM) geom.T {
	...
	pg.Rings = make([][]geom.PointM, len(s.Parts))
	jj := 0
	for i := 0; i < len(s.Parts); i++ {
		start, end := getStartEnd(s.Parts, s.Points, i)
		jj += end - start
		...
		for j := end - 1; j >= start; j-- {
			ss := s.Points[j]
			pg.Rings[i][j-start] = geom.PointM{ss.X, ss.Y, s.MArray[jj]}
			jj--
		}
	}
	return *pg
...
func polygonZ2geom(s shp.PolygonZ) geom.T {
	...
	pg.Rings = make([][]geom.PointZM, len(s.Parts))
	jj := 0
	for i := 0; i < len(s.Parts); i++ {
		start, end := getStartEnd(s.Parts, s.Points, i)
		jj += end - start
		...
		for j := end - 1; j >= start; j-- {
			ss := s.Points[j]
			pg.Rings[i][j-start] = geom.PointZM{ss.X, ss.Y, s.ZArray[j],
				s.MArray[jj]}
			jj--
		}
	}
...
func polyLine2geom(s shp.PolyLine) geom.T {
	...
	pl.LineStrings = make([]geom.LineString, len(s.Parts))
	for i := 0; i < len(s.Parts); i++ {
		start, end := getStartEnd(s.Parts, s.Points, i)
		pl.LineStrings[i].Points = make([]geom.Point, end-start)
		for j := start; j < end; j++ {
			pl.LineStrings[i].Points[j-start] = geom.Point(s.Points[j])
		}
	}
	return *pl
}
func polyLineM2geom(s shp.PolyLineM) geom.T {
	...
	pl.LineStrings = make([]geom.LineStringM, len(s.Parts))
	jj := 0
	for i := 0; i < len(s.Parts); i++ {
		start, end := getStartEnd(s.Parts, s.Points, i)
		...
		for j := start; j < end; j++ {
			ss := s.Points[j]
			pl.LineStrings[i].Points[j-start] =
...
func polyLineZ2geom(s shp.PolyLineZ) geom.T {
	...
	pl.LineStrings = make([]geom.LineStringZM, len(s.Parts))
	jj := 0
	for i := 0; i < len(s.Parts); i++ {
		start, end := getStartEnd(s.Parts, s.Points, i)
...
func multiPoint2geom(s shp.MultiPoint) geom.T {
	...
	mp.Points = make([]geom.Point, len(s.Points))
	for i, p := range s.Points {
		mp.Points[i] = geom.Point(p)
	}
	return *mp
}
func multiPointM2geom(s shp.MultiPointM) geom.T {
	mp := new(geom.MultiPointM)
	mp.Points = make([]geom.PointM, len(s.Points))
	for i, p := range s.Points {
		mp.Points[i] = geom.PointM{p.X, p.Y, s.MArray[i]}
	}
	return *mp
}
func multiPointZ2geom(s shp.MultiPointZ) geom.T {
	mp := new(geom.MultiPointZM)
	mp.Points = make([]geom.PointZM, len(s.Points))
	for i, p := range s.Points {
		mp.Points[i] = geom.PointZM{p.X, p.Y, s.ZArray[i], s.MArray[i]}
	}
	return *mp
}
...
func geom2polygon(g geom.Polygon) shp.Shape {
	parts := make([][]shp.Point, len(g.Rings))
...
func geom2polyLine(g geom.MultiLineString) shp.Shape {
	parts := make([][]shp.Point, len(g.LineStrings))
	for i, r := range g.LineStrings {
		parts[i] = make([]shp.Point, len(r.Points))
		for j, l := range r.Points {
...
func geom2polyLineM(g geom.MultiLineStringM) shp.Shape {
	...
	jj := 0
	for i, r := range g.LineStrings {
		m = append(m, make([]float64, len(r.Points))...)
		parts[i] = make([]shp.Point, len(r.Points))
		for j, l := range r.Points {
...
func geom2polyLineZ(g geom.MultiLineStringZM) shp.Shape {
	...
	for i, r := range g.LineStrings {
		m = append(m, make([]float64, len(r.Points))...)
		z = append(z, make([]float64, len(r.Points))...)
		parts[i] = make([]shp.Point, len(r.Points))
		for j, l := range r.Points {
			parts[i][j] = shp.Point{l.X, l.Y}
			m[jj] = l.M
